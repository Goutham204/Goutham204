# Hi, I'm Vishnu Goutham K

AI Engineer based in Dubai, UAE. I work with machine learning, deep learning, computer vision, and NLP, building end-to-end pipelines from data preprocessing to deployment.

---

## Skills

**Languages:** Python, SQL  
**Frameworks:** TensorFlow, PyTorch, Scikit-learn, Keras, OpenCV, NLTK  
**Deployment:** Flask, Streamlit, REST API  
**Tools:** Pandas, NumPy, Matplotlib, Jupyter, GitHub

---

## Projects

**[Smart Video Analytics System](https://github.com/Goutham204/Smart-Video-Analytics-System)**  
Serverless computer vision system deployed on AWS (Lambda, S3, DynamoDB, CloudFront) using YOLOv8 for real-time people counting and crowd alerts. Built a custom centroid-tracking algorithm for accurate IN/OUT line-crossing analytics. Supports live webcam and video upload.

**[Customer Complaint Analyzer](https://github.com/Goutham204/Customer-Complaint-Analyzer-System)**  
NLP-based complaint classification using an LSTM model with urgency detection and root-cause analysis. Flask REST API for real-time predictions and a Streamlit frontend for single and bulk CSV input.

**[AutoML Web Application](https://github.com/Goutham204/Smart_ML)**  
No-code ML platform built with Streamlit and Scikit-learn. Supports classification, regression, clustering, and PCA with automated EDA and model evaluation — no coding required from the user.

---

## Experience

**Junior AI Developer — Soften Technologies** *(Apr 2024 – Sep 2024)*  
Built and optimized ML/DL models for classification and regression tasks. Improved model accuracy by 30% through feature engineering and hyperparameter tuning. Reduced training time by 40% via pipeline optimization. Deployed models via Flask APIs and Streamlit apps.

---

## Education

- Diploma in Artificial Intelligence — Soften Technologies (2023–2024)  
- Bachelor of Computer Applications — University of Kerala (2018–2021)

---

## Contact

- Email: gouthamdm204@gmail.com  
- LinkedIn: [linkedin.com/in/vishnugoutham](https://linkedin.com/in/vishnugoutham) 
- Location: Dubai, UAE — available immediately (Visit Visa)
